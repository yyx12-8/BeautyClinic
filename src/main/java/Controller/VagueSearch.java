...
@WebServlet("/search")
public class VagueSearch extends HttpServlet {
    @Override
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        List<Map<String, Object>> sqlByCon =
                commonSelect.findSqlByCon("select * from product where type like concat('%',#{con},'%') or pro_introduce like concat('%',#{con},'%')", value);
        sqlSession.commit();
        if(sqlByCon.size() != 0){
